...
// Real preserved founder test evidence
const VALID_RECEIPT_1 = '81bba2e3-41d7-4472-91f1-c0ff8906eda6';
const VALID_ASSET_1 = '45be1276-437c-4f08-bf33-752bd29e9e4a'; // PNG

const VALID_RECEIPT_2 = 'fa5de83a-63b4-4974-9ca0-515978fec6ed';
const VALID_ASSET_2 = '78f4a438-08ea-4c9e-916c-739ee432b0e8'; // JPEG
...
async function runMatrix() {
  ...
  // A. missing passcode → DENIED
  {
    const { res } = await callRetrieve({ aid: VALID_ASSET_2, rid: VALID_RECEIPT_2 }, '10.1.0.1');
    recordTest('A', 'missing passcode → DENIED', 401, res.status, res.status === 401);
  }

  // B. wrong passcode → DENIED
  {
    const { res } = await callRetrieve({ aid: VALID_ASSET_2, rid: VALID_RECEIPT_2, passcode: 'invalid_passcode' }, '10.1.0.2');
    recordTest('B', 'wrong passcode → DENIED', 401, res.status, res.status === 401);
  }

  // C. malformed passcode → DENIED
  {
    const { res } = await callRetrieve({ aid: VALID_ASSET_2, rid: VALID_RECEIPT_2, passcode: TEST_PRINCIPAL_KEY + 'mod' }, '10.1.0.3');
  ...
  {
    const { res, data } = await callRetrieve({ aid: VALID_ASSET_2, rid: VALID_RECEIPT_2, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.4');
  ...
  // E. missing asset ID → DENIED
  {
    const { res } = await callRetrieve({ rid: VALID_RECEIPT_2, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.5');
    recordTest('E', 'missing asset ID → DENIED', 400, res.status, res.status === 400);
  }

  // F. missing receipt ID → DENIED
  {
    const { res } = await callRetrieve({ aid: VALID_ASSET_2, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.6');
  ...
  // G. random asset ID → DENIED
  {
    ...
    const { res } = await callRetrieve({ aid: randomAid, rid: VALID_RECEIPT_2, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.7');
  ...
  // H. random receipt ID → DENIED
  {
    ...
    const { res } = await callRetrieve({ aid: VALID_ASSET_2, rid: randomRid, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.8');
    recordTest('H', 'random receipt ID → DENIED', 403, res.status, res.status === 403);
  }

  // I. valid asset + wrong receipt → DENIED
  {
    const { res } = await callRetrieve({ aid: VALID_ASSET_2, rid: VALID_RECEIPT_1, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.9');
    recordTest('I', 'valid asset + wrong receipt → DENIED', 403, res.status, res.status === 403, 'Cross-tenant receipt mismatch rejected');
  }

  // J. valid receipt + wrong asset → DENIED
  {
    const { res } = await callRetrieve({ aid: VALID_ASSET_1, rid: VALID_RECEIPT_2, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.10');
    recordTest('J', 'valid receipt + wrong asset → DENIED', 403, res.status, res.status === 403);
  }

  // K. cross-inquiry asset → DENIED
  {
    const { res } = await callRetrieve({ aid: VALID_ASSET_1, rid: VALID_RECEIPT_2, passcode: TEST_PRINCIPAL_KEY }, '10.1.0.11');
    recordTest('K', 'cross-inquiry asset → DENIED', 403, res.status, res.status === 403);
  }

  // L. identifier-only request → DENIED
  {
    const req = new Request(`http://localhost:5202/api/submissions/assets/retrieve?aid=${VALID_ASSET_2}&rid=${VALID_RECEIPT_2}`, {
      method: 'GET',
    });
    const res = await assetsHandler(req);
  ...
  // M. direct public storage URL → DENIED
  {
    const rawStorageUrl = `${supabaseUrl.replace(/\/$/, '')}/storage/v1/object/founder-intake-assets/35efea72-bf9a-4632-9364-472d6e51ef5b/ff4701ee-c3bf-4562-8e27-7c43ad66a035-A30F54B1-1851-4368-AE9F-78CA5EA15672.jpeg`;
    const rawRes = await fetch(rawStorageUrl, { method: 'GET' });
    recordTest('M', 'direct public storage URL → DENIED', '400/403', rawRes.status, rawRes.status === 400 || rawRes.status === 403, 'Unsigned direct request rejected');
  }
  ...
  // N. direct anon RPC → DENIED
  {
    const anonRes = await fetch(`${supabaseUrl.replace(/\/$/, '')}/rest/v1/rpc/get_asset_by_id`, {
      ...
      body: JSON.stringify({ p_asset_id: VALID_ASSET_2 }),
    });
  ...
  // P. repeated failed authentication → RATE LIMITED
  {
    ...
    let lastCode = 0;
    for (let i = 1; i <= 6; i++) {
      const { res } = await callRetrieve({ aid: VALID_ASSET_2, rid: VALID_RECEIPT_2, passcode: 'bad' }, throttleIp);
      lastCode = res.status;
      if (res.status === 429) {
        lockedOut = true;
        break;
  ...
  // Q. successful authorized retrieval → one-object signed URL
  {
    const isSingleObject = Boolean(authorizedPayload?.signedUrl && authorizedPayload.signedUrl.includes('A30F54B1-1851-4368-AE9F-78CA5EA15672.jpeg'));
  ...
  // S. expired signed URL → DENIED
  {
    const path = '35efea72-bf9a-4632-9364-472d6e51ef5b/ff4701ee-c3bf-4562-8e27-7c43ad66a035-A30F54B1-1851-4368-AE9F-78CA5EA15672.jpeg';
    const signRes = await fetch(`${supabaseUrl.replace(/\/$/, '')}/storage/v1/object/sign/founder-intake-assets/${path}`, {
      method: 'POST',
      headers: {
        apikey: supabaseKey,
