import {useEffect,useState,lazy,Suspense,useCallback,type CSSProperties} from 'react';
...
export default function VisualEnvironmentLab(){
 ...
 const renderEnvironment=useCallback((state:CreationEnvironmentState)=><>
  <div className={cinematic&&state.position>=7&&state.position<8?'ve-ab-hidden':''}><Environment {...state}/></div>
  {cinematic&&state.position>=7&&state.position<8&&<div className="ve-cinematic-company"><Suspense fallback={<span className="ve-render-status">Initializing Company renderer…</span>}><CompanyRenderer reduced={state.reduced}/></Suspense></div>}
 </>,[cinematic]);
 useEffect(()=>{document.title='DWS — Company Renderer Benchmark';},[]);
 const choose=(value:boolean)=>{setCinematic(value);document.querySelector<HTMLButtonElement>('.p-stages button:last-child')?.click();};
 return <><Review52 environment={renderEnvironment}/><div className="ve-render-toggle" role="group" aria-label="Company environmental renderer"><button aria-pressed={!cinematic} onClick={()=>choose(false)}>Lightweight</button><button aria-pressed={cinematic} onClick={()=>choose(true)}>Cinematic</button></div></>;
}
