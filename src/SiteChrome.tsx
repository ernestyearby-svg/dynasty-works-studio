...
export function SiteHeader({portfolio=false}:{portfolio?:boolean}){const [open,setOpen]=useState(false);const menu=useRef<HTMLButtonElement>(null);return <header className="site-head" onKeyDown={e=>{if(e.key==='Escape'){setOpen(false);menu.current?.focus();}}}>
 ...
 <nav id="navigation" className={open?'navigation is-open':'navigation'} aria-label="Main navigation" onClick={()=>setOpen(false)}><a href={'/work'}>Work</a><a href={portfolio?'/capabilities':'#range'}>Capabilities</a><a href={portfolio?'/#creation':'#method'}>How we build</a><a href={'/studio'}>Studio</a><a href={'/contact'}>Contact</a><a className="nav-start" href={portfolio?'/#review-builder':'#builder'}>Start a company <Arrow/></a></nav>
 </header>}
