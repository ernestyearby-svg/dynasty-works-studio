...
function materialStyle(position:number):CSSProperties {
 ...
 const mix=Math.min(1,Math.max(0,(position-index-.6)/.4));
 const blend=(a:string,b:string,t:number)=>{const rgb=[1,3,5].map(i=>Math.round(parseInt(a.slice(i,i+2),16)*(1-t)+parseInt(b.slice(i,i+2),16)*t));return 'rgb('+rgb.join(',')+')'};
 const returnToPaper=Math.max(0,Math.min(1,position-8));
 const values=Object.fromEntries(materialKeys.map((key,i)=>[key,returnToPaper?blend(materials[7][i],materials[0][i],returnToPaper):blend(materials[index][i],materials[next][i],mix)]));
 const channels=values['--field'].match(/\d+/g)!.map(Number).map(v=>{const c=v/255;return c<=.04045?c/12.92:((c+.055)/1.055)**2.4});
 const luminance=channels[0]*.2126+channels[1]*.7152+channels[2]*.0722;
 // Switch foreground polarity at the accessible crossover instead of fading through low contrast.
 const crossover=luminance>=.12&&luminance<=.27;
 values['--drawing']=crossover?(luminance>.179?'#000000':'#ffffff'):luminance>.27?'#111214':'#ede9df';
 values['--secondary']=luminance>.45?'#484b48':values['--drawing'];
...
export default function Review51(){
 const [temperature,setTemperature]=useState(0);const [phase,setPhase]=useState(0);const origin=useRef<HTMLDivElement>(null);const destination=useRef<HTMLDivElement>(null);const [assembly,setAssembly]=useState(0);const [placement,setPlacement]=useState({left:0,top:0,width:0,height:0});const rail=useRef<HTMLElement>(null);const [reduced,setReduced]=useState(false);
 ...
 useEffect(()=>{let frame=0;const onScroll=()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(()=>{if(!rail.current)return;const r=rail.current.getBoundingClientRect();const length=rail.current.offsetHeight-innerHeight;const t=Math.max(0,Math.min(1,1-r.top/innerHeight));setAssembly(t);const progress=Math.max(0,-r.top/length);setTemperature(progress<=1?Math.min(7,progress*8):8+Math.min(1,(-r.top-length)/innerHeight));if(origin.current&&destination.current){const a=origin.current.getBoundingClientRect(),b=destination.current.getBoundingClientRect();const blend=t*t*(3-2*t);setPlacement({left:a.left+(b.left-a.left)*blend,top:a.top+(b.top-a.top)*blend,width:a.width+(b.width-a.width)*blend,height:a.height+(b.height-a.height)*blend})}setPhase(Math.max(0,Math.min(7,Math.floor((-r.top/length)*8))))})};window.addEventListener('scroll',onScroll,{passive:true});window.addEventListener('resize',onScroll);onScroll();return()=>{window.removeEventListener('scroll',onScroll);window.removeEventListener('resize',onScroll);cancelAnimationFrame(frame)}},[]);
 ...
 return <div className="p-prototype r51"><a className="p-skip" href="#creation">Skip to creation experience</a><section className="p-arrival" aria-labelledby="p-title"><header><a href="/v5-1-review/" className="p-wordmark">DYNASTY WORKS<span>STUDIO</span></a><span className="p-classification">Company creation studio</span><a href="#review-builder">Start a company <span aria-hidden="true">↗</span></a></header><div className="p-arrival-composition"><p className="p-arrival-note">The beginning of<br/>something that matters.</p><h1 id="p-title">From idea<span>to company.</span></h1><div className="p-origin-art"><div ref={origin} className="p-origin-anchor"/><span className="p-origin-caption"><i/>One idea. Infinite potential.</span></div><p className="p-arrival-bottom">Strategy. Identity. Product.<br/>Digital. Experience. Market.</p><div className="r51-arrival-actions"><a className="r51-action" href="#review-builder">Start a company <span aria-hidden="true">→</span></a><a className="p-enter" href="#operating">See how we build <span aria-hidden="true">↓</span></a></div></div></section><section ref={rail} className="p-evolution" id="creation" aria-label="Idea to company"><div className={"p-stage "+(temperature>=3.5&&temperature<8.5?"is-dark":"")} style={materialStyle(temperature)}><div className="p-stage-top"><span>DYNASTY WORKS / CREATION STUDY</span><span>Fictional demonstration</span></div><div className="p-stage-copy" aria-live="polite"><span className="p-count">0{phase+1}<span> / 08</span></span><h2>{statements[phase]}</h2><p>{descriptions[phase]}</p></div><div ref={destination} className="p-stage-art"/><nav className="p-stages" aria-label="Creation stages">{stages.map((s,i)=><button key={s} aria-current={i===phase?'step':undefined} onClick={()=>select(i)}><span className="p-stage-progress"/><small>0{i+1}</small>{s}</button>)}</nav><div className="p-stage-foot"><span>{phase===7?'The parts become the whole.':'Scroll to develop the idea.'}</span><span>IDEA → COMPANY</span></div></div></section><Operating/><ReviewBuilder/><div className="p-shared-art" style={{...placement,...materialStyle(temperature)}}><Artifact phase={phase} assembly={assembly}/></div></div>
}
...
function Operating(){
 const [active,setActive]=useState(0);const track=useRef<HTMLElement>(null);
 useEffect(()=>{let frame=0;const update=()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(()=>{const el=track.current;if(!el)return;const r=el.getBoundingClientRect();setActive(Math.max(0,Math.min(3,Math.floor(-r.top/(el.offsetHeight-innerHeight)*4))))})};addEventListener('scroll',update,{passive:true});update();return()=>{removeEventListener('scroll',update);cancelAnimationFrame(frame)}},[]);
 function go(i:number){const el=track.current;if(!el)return;scrollTo({top:el.getBoundingClientRect().top+scrollY+(el.offsetHeight-innerHeight)*(i+.15)/4,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'})}
 return <section ref={track} id="operating" className="r51-operating" aria-label="Four operating states"><div className="r51-operating-stage" data-operation={active}>
 <header><span>03 / THE OPERATING SYSTEM</span><span>The same company. Four operating states.</span></header>
 <nav aria-label="Operating states">{operations.map((o,i)=><button key={o.name} aria-current={active===i?'step':undefined} onClick={()=>go(i)}><small>0{i+1}</small>{o.name}<span aria-hidden="true">↗</span></button>)}</nav>
 <div className="r51-operating-art"><Artifact phase={7}/><div className="r51-extensions" aria-hidden="true"><span>Operations</span><span>New products</span><span>New markets</span></div></div>
 <div className="r51-operating-copy" aria-live="polite"><h2>{operations[active].title}</h2><p>{operations[active].terms}</p></div>
 <div className="r51-operating-foot"><span>One architecture, increasingly complete.</span><a href="#review-builder">Bring us the idea →</a></div>
 </div></section>
}
function ReviewBuilder(){
 const [started,setStarted]=useState(false);const [type,setType]=useState<BusinessType|null>(null);const [map,setMap]=useState<{phases:string[];complete:boolean}>({phases:[],complete:false});
 const title=useRef<HTMLHeadingElement>(null);
 useEffect(()=>{if(started&&!type)title.current?.focus()},[started,type]);
 const active=(name:string)=>map.phases.some(p=>name==='strategy'?p==='Foundation':name==='identity'?p==='Brand':name==='product'?p==='Product / Infrastructure':name==='digital'?p==='Digital':name==='experience'?p==='Activation':name==='market'?['Launch','Distribution','Commercialization'].includes(p):['Automation System','Growth'].includes(p));
 const classes=['strategy','identity','product','digital','experience','market','systems'].filter(active).map(s=>'has-'+s).join(' ');
 return <section id="review-builder" className={'r51-builder '+(map.complete?'has-result':'')}>
 <header><span>04 / COMPANY BUILDER</span><h2>Bring us<br/><em>the idea.</em></h2><p>This is how we build.<br/>Now, make it yours.</p></header>
 <div className="r51-instrument">
 <div className="r51-instrument-status"><span>DYNASTY WORKS / FOUNDER DIAGNOSTIC</span><span>{map.complete?'OUTPUT / INITIAL ROADMAP':type?'INPUT / YOUR COMPANY':started?'01 / COMPANY TYPE':'READY / YOUR IDEA'}</span></div>
 <div className="r51-response"><div className={'r51-response-art '+classes+(started?' is-active':'')}><Artifact phase={started?7:0}/>{type&&<div className="r51-mobile-map">{map.phases.map((p,i)=><div key={p}><small>{String(i+1).padStart(2,'0')}</small><span>{p}</span><i aria-hidden="true"/></div>)}</div>}</div><p>{map.complete?'Your mapped disciplines':type?'Initial engine response':'One idea. Your starting point.'}</p>{type&&<ul className="r51-response-labels">{map.phases.map(p=><li key={p}>{p}</li>)}</ul>}</div>
